...
#ifdef	FROM_REAR_AF_CAL_MACRO_ADDR
...
char supported_camera_ids[] = {
	0,  //REAR_0 = Rear Wide
	1,  // FRONT WIDE 
	50,  //FRONT WIDE IN TABS7FE ELSE REAR_UW
	52,
	54,
};
			...
			size_t count)
{
	...
	if(get_hw_rev_for_vois() >= 10) {
		sm5714_store(buf);
		return count;
	}
#endif
#if IS_ENABLED(CONFIG_LEDS_S2MU106_FLASH) && IS_ENABLED(CONFIG_SEC_A23XQ_PROJECT)
	if(get_hw_rev_for_vois() < 10){
		s2mu106_store(dev,attr,buf,count);
	}else{
#if defined(CONFIG_MACH_A23XQ_USA_TFN) || defined(CONFIG_MACH_A23XQ_USA_OPEN) || defined(CONFIG_MACH_A23XQ_USA_SINGLEW) || defined(CONFIG_MACH_A23XQ_USA_SINGLE)
		s2mu106_store(dev,attr,buf,count);
#endif
	}
#endif
